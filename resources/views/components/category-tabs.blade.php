<ul
    ...
    <li class="me-2">
        <a href="/" class="{{ request('category') ?  'inline-block px-4 py-2 rounded-lg hover:text-gray-900 hover:bg-gray-100 dark:hover:bg-gray-800 dark:hover:text-white'
    ...
    @forelse ($categories as $category)
        <li class="me-2">
            <a href="{{ route('post.byCategory', $category->id) }}"
                class="{{ Route::currentRouteName() == 'post.byCategory' && request()->route('category')?->id == $category->id 
                ? 'inline-block px-4 py-2 text-white bg-blue-600 rounded-lg active' 
