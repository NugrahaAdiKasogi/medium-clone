<div class="flex mt-6 bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
    {{-- Post Details --}}
    <div class="p-5 flex-1">
        ...
        <div class="py-4">
            <span
                class="inline-block px-3 py-1 text-sm font-semibold text-gray-700 bg-gray-200 rounded-full dark:bg-gray-700 dark:text-gray-300">
                {{ $post->category->name }}
            </span>
        </div>
        ...
        <div class="flex items-center justify-between text-sm text-gray-500 dark:text-gray-400">
            <div>
                published by 
                <a href="{{ route('profile.show', $post->user->username) }}" class="underline hover:text-gray-700 dark:hover:text-gray-300">
                    {{ $post->user->name }}
                </a> 
                on {{ $post->formatted_published_at }}
            </div>
    ...
    <a href="{{ route('post.show', [
        ...
        <img class="rounded-r-lg w-64 h-full  max-h-64 ms-6" src="{{ $post->imageUrl('preview') }}" alt="image" />
    </a>
</div>